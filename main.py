...
log_folder = os.path.join(external_path, 'Documents', 'Storke_LLC_App', 'DO NOT DELETE')
log_file = os.path.join(log_folder, "log.txt")


#Make sure that there is a log.txt file
try:
...
megger_test_folder = os.path.join(external_path, "Documents", "Storke_LLC_App", "Megger Test Files")
os.makedirs(megger_test_folder, exist_ok=True) 
        ...
        if customer_name:
            ...
            if phase_1 and phase_2 and phase_3:
                ...
                if megger_test != '':
                    ...
                    #Makes directory to the megger test folder
                    megger_test_file = os.path.join(megger_test_folder, file_name)
                    try:
                        #Uses an existing test file
                        if os.path.exists(megger_test_file):
                            log('The megger test file exists\n')
                            workbook = load_workbook(megger_test_file)
                            sheet = workbook.active
                            workbook.save(megger_test_file)
                            log('Loaded previously made megger test file\n')
                        #Makes a new test file
                        else:
                            workbook = Workbook()
                            sheet = workbook.active
                            sheet.title = "Sheet1"
